...
export async function getProgramTokenAccounts(
  connection: Connection,
  publicKey: PublicKey,
  config?: GetProgramAccountsConfig
): Promise<ProgramAccount<TokenAccount>[]> {
  const results = await connection.getParsedProgramAccounts(publicKey, config);
  return results.map((r) => {
    const publicKey = r.pubkey;
    const data = Buffer.from(r.account.data as ArrayBuffer);
